...
import {
  ...
  Ticket, Trash2, Plus, ArrowRight, MessageSquare, Clock,
  CreditCard, Phone, Mail, Calendar, Hash, Tag, Building2,
  Zap, ShoppingCart, ArrowDownCircle, ArrowUpCircle, Wallet, Edit3, X,
} from 'lucide-react';
...
export function DealDetailSheet({ deal, onClose, onUpdate, onDelete }: Props) {
  ...
  useEffect(() => {
    ...
    ticketPaymentService.getByDeal(deal.id).then(setPayments).catch(() => {});
  }, [deal?.id]);
          ...
          <div className="relative flex-shrink-0 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-950 text-white overflow-hidden">
            ...
            <div className="relative px-6 pt-5 pb-4">
              ...
              <div className="flex items-start justify-between gap-3 mb-4">
                ...
                <button
                  onClick={handleClose}
                  className="flex-shrink-0 h-8 w-8 flex items-center justify-center rounded-full bg-white/10 hover:bg-white/20 transition-colors"
                >
                  <X className="h-4 w-4" />
                </button>
              </div>
              ...
              {isAdminOrManager && (
                <div className="flex gap-2 mt-3">
                  <Button
                    ...
                    className="h-8 text-[11px] gap-1.5 flex-1 bg-transparent border-white/20 text-white hover:bg-white/10 hover:text-white"
                    onClick={() => handleWhatsApp('seller')}
                  >
                    <MessageSquare className="h-3 w-3 text-green-400" /> WA Seller
                  </Button>
                  <Button
                    size="sm" variant="outline"
                    className="h-8 text-[11px] gap-1.5 flex-1 bg-transparent border-white/20 text-white hover:bg-white/10 hover:text-white"
                  ...
                  >
                    <MessageSquare className="h-3 w-3 text-green-400" /> WA Buyer
                  </Button>
                  {canDelete && (
                    <Button
                      size="sm" variant="outline"
            ...
            <div className="p-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="rounded-2xl border bg-card p-4 space-y-1.5">
                      ...
                      <Row icon={Phone} label="Phone" value={
                        <a href={`tel:${deal.seller.phone}`} className="hover:text-primary transition-colors">{deal.seller.phone}</a>
                      } />
                  ...
                  {(deal.seller.purchasePrice || deal.seller.purchasedFrom) && (
                    <div className="rounded-2xl border bg-card p-4 space-y-1.5">
                      ...
                      {deal.seller.purchasePrice && <Row icon={IndianRupee} label="Our Cost" value={`₹${deal.seller.purchasePrice.toLocaleString('en-IN')}/seat`} />}
                  ...
                  <Button
                    ...
                    onClick={() => handleWhatsApp('seller')}
                  >
                    <MessageSquare className="h-4 w-4" /> Message Seller on WhatsApp
                  </Button>
                </div>
              )}
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="rounded-2xl border bg-card p-4 space-y-1.5">
                      <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-2">Contact</p>
                      <Row icon={Phone} label="Phone" value={
                        <a href={`tel:${deal.buyer.phone}`} className="hover:text-primary transition-colors">{deal.buyer.phone}</a>
                      } />
                  ...
                  {(deal.buyer.agreedPricePerSeat || deal.buyer.totalCollected || deal.buyer.paymentRef) && (
                    <div className="rounded-2xl border bg-card p-4 space-y-1.5">
                      ...
                      {deal.buyer.agreedPricePerSeat && <Row icon={IndianRupee} label="Agreed"    value={`₹${deal.buyer.agreedPricePerSeat.toLocaleString('en-IN')}/seat`} />}
                      {deal.buyer.totalCollected     && <Row icon={CreditCard}  label="Collected" value={`₹${deal.buyer.totalCollected.toLocaleString('en-IN')}`} accent="text-emerald-600 dark:text-emerald-400" />}
                  ...
                  <Button
                    ...
                    onClick={() => handleWhatsApp('buyer')}
                  >
                    <MessageSquare className="h-4 w-4" /> Message Buyer on WhatsApp
                  </Button>
                </div>
              )}
              ...
              {activeTab === 'payments' && (
                <div className="space-y-4">
                  ...
                  {canEdit && (
                    <Button variant="outline" className="w-full gap-1.5 h-10 rounded-xl" onClick={() => setPaymentOpen(true)}>
              ...
              {activeTab === 'edit' && canEdit && (
                <div className="space-y-5">
                  ...
                  <Button className="w-full h-11 rounded-xl font-semibold" onClick={handleSave} disabled={saving}>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete the deal between{' '}
              <strong>{deal?.seller.brokerName}</strong> and{' '}
              <strong>{deal?.buyer.brokerName}</strong>.{' '}
